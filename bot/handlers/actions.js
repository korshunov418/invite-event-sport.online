...
async function updateEventMessage(ctx, eventId, chatId, eventInfo) {
  try {
    ...
    if (lastMessage) {
      ...
      const formattedMessage = formatEventMessage(eventInfo, participants, lang);
      ...
      try {
        ...
        Logger.info(`Сообщение обновлено: event=${eventId}, participants=${participants.length}`);
      } catch (error) {
        Logger.error(`Ошибка редактирования сообщения: ${error}`);
        // Если сообщение нельзя отредактировать (например, слишком старое),
        // можно отправить новое сообщение или просто проигнорировать ошибку
      }
    } else {
      Logger.warn(`Не найдено сообщение для обновления: event=${eventId}, chat=${chatId}`);
    }
  } catch (error) {
